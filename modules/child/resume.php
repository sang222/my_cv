<section class="section-strength section-content" page="resume">
  <div class="wrapper_title">
    <h2 class="title">resume</h2>
    <span>1 year of experience</span>
  </div>
  <div class="row">

    <!-- education -->
    <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12 row__education">
      <h3 class="title_small">Education</h3>
      <div class="row__education--content">
      <h4>2004</h4>
        <h5>Diploma In UI/UX Design</h4>
        <p>Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam.</p>
      </div>
      <div class="row__education--content">
      <h4>2004</h4>
        <h5>Diploma In UI/UX Design</h4>
        <p>Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam.</p>
      </div>
      <div class="row__education--content">
      <h4>2004</h4>
        <h5>Diploma In UI/UX Design</h4>
        <p>Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam.</p>
      </div>
    </div>
    <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12 row__education">
      <h3 class="title_small">Experence</h3>
      <div class="row__education--content">
      <h4>2016</h4>
        <h5>Bliss Interactive</h4>
        <p>Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam.</p>
      </div>
    </div>
    <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12 coding">
      <h3 class="title_small">Skills</h3>
      <div class="coding_bar">
        <h5>html5</h5>
        <div class='progress-bar-holder'>
          <div class='progress-bar-blue' process="95"></div>
        </div>
      </div>
      <div class="coding_bar">
        <h5>CSS3</h5>
        <div class='progress-bar-holder'>
          <div class='progress-bar-blue' process="93"></div>
        </div>
      </div>
      <div class="coding_bar">
        <h5>JQuery</h5>
        <div class='progress-bar-holder'>
          <div class='progress-bar-blue' process="85"></div>
        </div>
      </div>
      <div class="coding_bar">
        <h5>Wordpress</h5>
        <div class='progress-bar-holder'>
          <div class='progress-bar-blue' process="70"></div>
        </div>
      </div>
    </div>
  </div>
</section>
